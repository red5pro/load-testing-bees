...
#
# USAGE: rtmpbee-publisher.sh [endpoint] [app] [streamName] [amoun_of_streams_to_start] [amount_of_time_to_playback] [Red5pro_server_API_key] [mp4-file] 
#
...
# 
# LOCAL EXAMPLE: ./rtmpbee-publisher.sh localhost live stream1 10 10 abc123 /path_to_video_file/bbb_480p.mp4 false
#
...
if [[ -z "$endpoint" || -z "$port" || -z "$app" || -z "$stream_name" || -z "$amount" || -z "$timeout" || -z "$api_key" || -z "$file" || -z "$audio_only" ]]; then
    ...
    log_w "Example: ./rtmpbee-publisher.sh your.red5pro-deploy.com 1935 live stream1 10 10 abc123 /path_to_video_file/bbb_480p.mp4 false"
    exit 1
fi
