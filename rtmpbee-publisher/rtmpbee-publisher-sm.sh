...
#
# EXAMPLE FOR VIDEO FILE: ./rtmpbee-publisher-sm.sh red5pro.server.com example_user example_password my_nodegroup 1935 live stream1 1 60 /path_to_video_file/bbb_480p.mp4 false
# EXAMPLE FOR AUDIO FILE: ./rtmpbee-publisher-sm.sh red5pro.server.com example_user example_password my_nodegroup 1935 live stream1 1 60 /path_to_audio_file/audio.wav true
# 
# LOCAL EXAMPLE: ./rtmpbee-publisher-sm.sh localhost live stream1 10 100 /path_to_video_file/bbb_480p.mp4 false      #This will publish 10 Streams for 100 seconds with audio
#
...
mkdir -p "${current_dir}"
log_file="${current_dir}/rtmp_sm_pub"
PIDS=()
...
if [[ -z "$sm_username" || -z "$sm_password" || -z "$nodegroup_name" || -z "$endpoint" || -z "$port" || -z "$app" || -z "$stream_name" || -z "$amount" || -z "$timeout" || -z "$file" || -z "$audio_only" ]]; then
    log_w "Not all arguments are set. Please check your command."
    log_w "USAGE: ./rtmpbee-publisher-sm.sh [endpoint] [SM_username] [SM_password] [Nodegroup_name] [rtmp_port] [app] [streamName] [amoun_of_streams_to_start] [amount_of_time_to_playback_in_seconds] [mp4-file] [boolean_for_audio_only]"
    log_w "Example: ./rtmpbee-publisher-sm.sh your.red5pro-deploy.com example_username example_password your_nodegroup_name 1935 live stream1 10 10 /path_to_video_file/bbb_480p.mp4 false"
    exit 1
fi
...
for ((i=1;i<=amount;i++)); do
    ...
    target="rtmp://${origin_node}:${port}/${app}/${name}"
