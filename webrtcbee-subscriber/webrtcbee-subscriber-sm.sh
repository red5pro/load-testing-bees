...
#===================================================================================
#
# FILE: webrtcbee-subscriber-sm.sh
#
# USAGE: webrtcbee-subscriber-sm.sh [endpoint] [SM_username] [SM_password] [stream_name] [amount_of_subscribers] [amount_of_time_to_playback_stream_in_seconds]
#
# EXAMPLE: ./webrtcbee-subscriber-sm.sh "https://your.server.com/red5/proxy-subscriber.html?host=your.server.com&protocol=wss&port=443&whipwhep=true&verbose=1" SM_username SM_password stream1 5 30
#
# DESCRIPTION: Creates N-number of headless WebRTC-based subscriptions to a live stream.
# Console output sent to log/webrtc_sm_sub_N.log and monitored for status.
#
...
if [[ -z "$sm_username" || -z "$sm_password" || -z "$endpoint" || -z "$amount" || -z "$timeout" || -z "$stream_name" ]]; then
    log_w "Not all arguments are set. Please check your command."
    log_w "USAGE: webrtcbee-subscriber-sm.sh [endpoint] [SM_username] [SM_password] [stream_name] [amount_of_subscribers] [amount_of_time_to_playback_stream_in_seconds]"
    log_w 'Example: webrtcbee-subscriber-sm.sh "https://your.server.com/red5/proxy-subscriber.html?host=your.server.com&protocol=wss&port=443&whipwhep=true&verbose=1" SM_username SM_password stream1 1 60'
    exit 1
fi
...
endpoint_with_params="${endpoint}&smuser=${sm_username}&smpass=${sm_password}&streamName=${stream_name}"
