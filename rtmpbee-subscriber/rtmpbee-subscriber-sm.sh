...
#
# EXAMPLE: ./rtmpbee-subscriber-sm.sh red5pro.server.com my_nodegroup 1935 live stream1 1 60 
#
...
if [[ -z "$nodegroup_name" || -z "$endpoint" || -z "$port" || -z "$app" || -z "$stream_name" || -z "$amount" || -z "$timeout" ]]; then
    ...
    log_w "USAGE: ./rtmpbee-subscriber-sm.sh [endpoint] [Nodegroup_name] [rtmp_port] [app] [streamName] [amount_of_subscribers] [amount_of_time_to_playback_in_seconds] "
    log_w "Example: ./rtmpbee-subscriber-sm.sh your.red5pro-deploy.com your_nodegroup_name 1935 live stream1 10 100"
    exit 1
fi
